#!/usr/bin/env scala
// Scala 3 script: tools/scaladoc_checker.scala
// Usage: scala tools/scaladoc_checker.scala -- --dry-run | --apply [--root PATH] [--exclude pat1,pat2]
  ...
  if args.contains("--help") || args.contains("-h") then
    Console.err.println(
      """Scaladoc Checker
        ...
        |
        |Examples:
        |  scala tools/scaladoc_checker.scala -- --dry-run
        |  scala tools/scaladoc_checker.scala -- --apply --root library --exclude generated,tmp
        |""".stripMargin)
    System.exit(0)

  val cfg = ScaladocChecker.parseArgs(args.toArray)
  val exit = ScaladocChecker.run(args.toArray)
  System.exit(exit)