...
abstract class Enumeration (initial: Int) extends Serializable {
  ...
  class ValueSet private[ValueSet] (private var nnIds: immutable.BitSet)
    ...
    /** Returns an iterator over the values of this set that are not below a threshold
     *  derived from `start`, in increasing order of their ids. Unlike the other methods
     *  of this class, this one does not subtract the enumeration's lowest id from
     *  `start`'s id before comparing it against the stored ids, so the threshold is
     *  `start`'s id when this enumeration uses no negative ids, and lies below `start`
     *  when it does. Iteration begins at the least value of this set that is not below
     *  the threshold; `start` itself need be neither an element of this set nor the
     *  first value yielded.
     *
     *  @param start the value from whose id the starting threshold is derived
     *  @note NEEDS-HUMAN: is the missing adjustment by the enumeration's lowest id
     *        intentional here, given that `contains`, `incl`, `excl`, and `iterator`
     *        all apply it? If it is a bug, this comment should instead state the plain
     *        `SortedSet.iteratorFrom` contract, that iteration begins at the least
     *        value of this set that is not below `start`.
     */
